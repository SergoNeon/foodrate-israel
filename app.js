...
function placeCard(p){return '<a class="card" href="restaurant.html?id='+encodeURIComponent(p.id)+'"><div class="cover"><img src="'+esc(imageFor(p.category))+'" alt="'+esc(placeName(p))+'" loading="lazy"><span class="badge">'+esc(categoryName(p.category))+'</span></div><div class="cardbody"><div class="cardtop"><div><h3>'+esc(placeName(p))+'</h3><div class="meta">'+esc(categoryName(p.category))+' · '+esc(cityName(p))+'</div></div></div><div class="tags">'+(p.phone?esc(p.phone)+' · ':'')+esc(p.category)+'</div><div class="dishrow"><span>'+tr('details')+'</span><strong>→</strong></div></div></a>'}
